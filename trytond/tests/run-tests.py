...
parser.add_argument("-n", "--names", help="filter tests by their names."
    " For example: view,button")
...
names = opt.names.split(',') if opt.names else None
...
if not opt.modules:
    ...
    suite = modules_suite(opt.tests, doc=opt.doctest, names=names)
